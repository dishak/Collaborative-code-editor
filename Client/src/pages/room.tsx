  ...
  useEffect(() => {
    ...
    newSocket.onmessage = (message) => {
      ...
      if (parsedMessage.Title === "Room-Info") {
        ...
        setSubmitClicked(false);
        alert(
          "Code cannot be processed now cause it requires a continiously running worker service whcih is expensive 😅😅, if you want to, you can clone the repo and run worker process locally!!",
        );
      }
    };
    setSocket(newSocket);
    ...
    <Flex h="100vh" color="blue.200">
      ...
      {isLargeScreen && (
        <Flex direction="column" gap="30px" bg="blue.900" w="20%" h="full">
          <Box p="4">
            <div className="text-4xl text-center">Members</div>
          </Box>
      ...
      <Flex
        ...
        bg="gray.900"
        flex="1"
        width={useBreakpointValue({ base: "100vh", md: "80vh", sm: "60vh" })}
      >
        {/* Header Bar */}
        <Flex
          bg="gray.800"
      ...
      {!isLargeScreen && (
        <Drawer placement="right" onClose={onRightClose} isOpen={isRightOpen}>
          ...
          <DrawerContent bg="blue.900">
            <DrawerCloseButton />
            <DrawerHeader>
              <div className="text-4xl text-center">Chats</div>
            </DrawerHeader>
            <DrawerBody>
